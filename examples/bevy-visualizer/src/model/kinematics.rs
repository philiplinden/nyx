/*!
 * Physics entities and settings.
 *
 * NOTE: This module is ripped from particular's `pocket-solar-system` example.
 *
 * This module is responsible for updating motion, regardless of its cause.
 * It is mostly an interface to Bevy's physics engine. As written, we should be
 * able to modelpretty much any situation by initializing an entity's position
 * and velocity then apply accelerations to move it around.
 *
 * One of the critical roles of this module is managing the simulation time.
 * There are two settings for this: `delta_time` and `time_scale`. The former
 * represents the span of time between simulation steps. The latter lets us
 * decouple the simulation's timescale from the visualizer's. This drives the UI
 * controls to fast-forward, slow-mo, and pause the simulation.
 */
