/**
 * This module sets up the UI: dialog boxes, buttons, sliders, text, etc.
 * This means the UI must react to changes in entities (labels should move with
 * their parents, for example) and UI elements must influence the entities and
 * underlying propagators (pause and resume, changing time scale, etc).
 *
 * The vast majority of this module was ripped directly from the particular.rs
 * example `pocket-solar-system`.
 */
