use bevy::prelude::*;
use bevy_rapier3d::prelude::{Velocity, ReadMassProperties};
use particular::prelude::*;

use crate::DT;
use crate::physics::schedule::PreRapierSchedule;

pub const COMPUTE_METHOD: sequential::BruteForceSIMD = sequential::BruteForceSIMD;
...
fn accelerate_particles(mut query: Query<(&mut Velocity, &Transform, &ReadMassProperties)>) {
    query
        ...
        .map(Vec3::from)
        .zip(&mut query)
        .for_each(|(acceleration, (mut velocity, ..))| velocity.linvel += acceleration * DT);
}
